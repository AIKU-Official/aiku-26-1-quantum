...
2026년 1학기 고려대학교 딥러닝 학회 **AIKU** 프로젝트입니다.
...
PennyLane (default.qubit, autograd backend), numpy, matplotlib, scikit-learn.

## 사용 방법

`scripts/` 의 워커를 순서대로 실행하면 데이터 점검 → baseline 학습 → residual 진단 → ablation 비교가 재현됩니다. 결과 그림은 `figures/` 에 있습니다.
...
- 위 성공은 4색 데이터 × 4-level 각도 인코딩 × Z₄ 진단 basis가 정확히 정렬된 toy 설정에 의존. 임의 데이터로의 일반화는 검증 중.
- Prescribed 우위에는 "올바른 feature pairing"과 "pooling 깊이 생존"이 함께 기여하며 완전 분리는 미완.
- 통계는 3 seed 기준.

## 후속

임의 구조(고차 상관·3파티)로 일반화, 쌍별 얽힘 세기 처방, 실제 이미지 데이터 적용.
...
Y. Kim, K. Hwang, H. Kwon, and Y. Kim, "The power of entanglement in distributed quantum machine learning," arXiv:2605.03864, 2026.
