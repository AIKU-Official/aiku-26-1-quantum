# CQ2/T1a 워커: separable(rank-1 cross) 합성데이터에 cond f0(Bell-0) baseline 학습.
#   residual 진단의 f0 로 쓰임. cache/cond/separable/ 에 per-seed 저장(best 는 진단 스크립트에서).
...
ds = S.make_separable()
Xang = S.angle_encode(ds["X"]); y = ds["y"]
...
sdir = circuits.cache_subdir("separable", a.method)
np.savez(os.path.join(sdir, f"separable_pool_s{a.seed}.npz"),
         params=params, n_q=nq, pooling=True, reupload=True, n_blocks=a.n_blocks,
         dataset="separable", seed=a.seed, method=a.method,
         train_acc=tr_acc, test_acc=te_acc, train_idx=tr["idx"], test_idx=te["idx"])
print(json.dumps({"dataset": "separable", "seed": a.seed, "method": a.method,
